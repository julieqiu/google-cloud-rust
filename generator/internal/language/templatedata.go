...
func NewTemplateData(model *api.API, config map[string]string, output, language, dir string) (string, []any, error) {
	var data *templateData
	switch language {
	case "rust":
		c, err := newRustCodec(model, output, config)
		if err != nil {
			return "", nil, err
		}
		data = &templateData{
	...
	if data.AdditionalContext() != nil {
		context = append(context, data.AdditionalContext())
	}
	root := filepath.Join(dir, data.TemplateDir())
...
func (t *templateData) TemplateDir() string {
	return t.c.TemplateDir()
}

func (t *templateData) AdditionalContext() any {
	return t.c.AdditionalContext()
}

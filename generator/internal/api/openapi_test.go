...
`
	...
	checkOpenAPIMessage(t, *message, Message{
		Name:          "Automatic",
		ID:            "..Automatic",
...
`
	...
	checkOpenAPIMessage(t, *test.Messages[0], Message{
		Name:          "Fake",
		ID:            "..Fake",
		Documentation: "A test message.",
		Fields: []*Field{
...
const sampleOpenAPIJSON = "../testing/testdata/openapi/secretmanager_openapi_v1.json"

func TestOpenAPI_MakeAPI(t *testing.T) {
	contents, err := os.ReadFile(sampleOpenAPIJSON)
	if err != nil {
		t.Fatal(err)
	}
	model, err := createDocModel(contents)
	...
	checkOpenAPIMessage(t, *location, Message{
	...
	checkOpenAPIMessage(t, *listLocationsResponse, Message{
	...
	checkOpenAPIMessage(t, *listLocationsRequest, Message{
	...
	checkOpenAPIMessage(t, *secretPayload, Message{
		Name:          "SecretPayload",
		ID:            "..SecretPayload",
	...
	checkOpenAPIMethod(t, service, "ListLocations", &Method{
		Name:          "ListLocations",
	...
	checkOpenAPIMethod(t, service, "CreateSecret", &Method{
		Name:          "CreateSecret",
	...
	checkOpenAPIMethod(t, service, "AddSecretVersion", &Method{
...
func TestOpenAPI_SyntheticMessageWithExistingRequest(t *testing.T) {
	...
	checkOpenAPIMessage(t, *setIamPolicyRequest, Message{
...
func checkOpenAPIMethod(t *testing.T, service *Service, name string, want *Method) {
	t.Helper()
...
func checkOpenAPIMessage(t *testing.T, got Message, want Message) {
	t.Helper()
	if diff := cmp.Diff(want, got, cmpopts.IgnoreFields(Message{}, "Fields")); diff != "" {
